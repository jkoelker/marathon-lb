...
DCOS_E2E_CONTAINER_REPO ?= mlb-dcos-e2e-devkit
DCOS_E2E_CONTAINER_TAG ?= latest
DCOS_E2E_CONTAINER_NAME ?= mlb-dcos-e2e-devkit
...
DCOS_E2E_IMG := $(DCOS_E2E_CONTAINER_REPO):$(DCOS_E2E_CONTAINER_TAG)
...
help:
	...
	@echo ""
	@echo "    image, dcos-e2e, devkit"
	@echo "        Build the marathon-lb/dcos-e2e/devkit images"
	@echo ""
	@echo "    dcos"
	@echo "        Start a dcos cluster with dcos-e2e"
	@echo ""
	@echo "    dcos-shell, shell"
	@echo "        Run /bin/bash in a dcos-e2e/devkit container"
	@echo ""
	@echo "    test"
...
cluster-public-ip: dcos-e2e $(CLUSTER_TARGET) 
ifeq ($(strip $(PUBLIC_AGENT_IP)),)
...
.PHONY: cluster-url
cluster-url: dcos-e2e $(CLUSTER_TARGET)
ifeq ($(strip $(CLUSTER_URL)),)
...
.PHONY: dcos-e2e
dcos-e2e:
	@echo "+ Build dcos-e2e image"
	@docker build \
		--rm \
		--quiet \
		--force-rm \
		--file $(MLB_PATH)/Dockerfile.dcos-e2e \
		--tag $(DCOS_E2E_IMG) \
		$(MLB_PATH) > /dev/null 2>&1 || true
